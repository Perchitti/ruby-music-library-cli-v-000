...
while input != "exit"
 ...
 input = gets.strip
      ...
      when 'play song'
        play_song
        #   #gets- grab the last input from the user, .strip - Returns a copy of str with leading and trailing whitespace removed.
      end
      end
...
artists.each_with_index do |a, index|
  puts "#{index + 1}. #{a.name}"
  end
end
...
genres.each_with_index do |g, index|
  puts "#{index + 1}. #{g.name}"
  end
end
